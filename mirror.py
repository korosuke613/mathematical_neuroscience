...
class Network:
    def __init__(self, input_array_):
        self._i_layer = InputLayer(input_array_)
        self._h_layer = HiddenLayer()
        self._o_layer = OutputLayer()
        self.learning_rate = 0.005
        self.delta = list()

    def check(self, binary):
        self._i_layer.neuron = np.array([float(binary[0]),
                                         float(binary[1]),
                                         float(binary[2]),
                                         float(binary[3]),
                                         float(binary[4]),
    ...
    def forward(self):
        ...
        self._h_layer.neuron = Z1
        self._o_layer.neuron = y
    ...
    def back_propagation(self):
        W1 = self._i_layer.w
        W2 = self._h_layer.w
        y = self._o_layer.neuron
        Z1 = self._h_layer.neuron
        X = self._i_layer.neuron

        if self._i_layer.correct is True:
            d = 1.0
        else:
            d = 0.0
...
class InputLayer(Layer):
    def __init__(self, input_array: dict):
        super().__init__()
        self.correct = np.array([input_array["is_mirror"]])
        ...
        self.w = 0.01 * np.random.randn(6, 2)
...
class HiddenLayer(Layer):
    def __init__(self):
        ...
        self.w = 0.01 * np.random.randn(2, 1)
...
def main():
    ...
    for epoch in range(1000):
        # 1. 回路に入力を与える
        new._i_layer.set_array(mirror[epoch % 64])
        new.forward()
        new.back_propagation()
    new.check("010010")
